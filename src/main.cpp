...
#if defined(NDEBUG)
# error "Erikcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x96e4e4c0bef179ae6c78776d38a24ff07efd26016a8e77ee697af592de22634d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Erikcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Erikcoin Signed Message:\n";
...
{
    // Erikcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//   }
    ...
    nSubsidy >>= (nHeight / 5040000); // Erikcoin: 5040k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Erikcoin: 1 minutes
static const int64 nTargetSpacing = 1 * 60; // Erikcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Erikcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Erikcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x96e4e4c0bef179ae6c78776d38a24ff07efd26016a8e77ee697af592de22634d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "29th of Januari 2017. Weather is nice in Rotterdam! Penguins are cool!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042c6970da340da261778f6dee5d0cb0a9f2def7e90086a2accf0a86f5865aa04b3e722b3566859de419ff26ca75a3a998b3dc854f61204f09b967ff0af7a5d16e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485697423;
        ...
        assert(block.hashMerkleRoot == uint256("0x647d83ecab38d4af70137bff4e8cb00b923c8e839b8afd3214cfbf4383e952ef"));
...
unsigned char pchMessageStart[4] = { 0xd2, 0xa0, 0xe6, 0xdb }; // Erikcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ErikcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ErikcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ErikcoinMiner(CWallet *pwallet)
{
    printf("ErikcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("erikcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ErikcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ErikcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ErikcoinMiner, pwallet));
}
