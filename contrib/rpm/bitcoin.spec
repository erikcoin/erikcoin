...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1348
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1349
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11348
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11349
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 1348
	%{_sbindir}/semanage port -d -p tcp 1349
	%{_sbindir}/semanage port -d -p tcp 11348
	%{_sbindir}/semanage port -d -p tcp 11349
	for selinuxvariant in %{selinux_variants}; do
